...
class SocraticJudge:
        ...
        for attempt in range(max_retries):
            try:
                params = dict(
                    ...
                    max_completion_tokens=8192,  # FIXED: gpt-5.5 is a reasoning model; 512 was consumed by reasoning tokens -> empty content (finish_reason=length)
    ...
    def _call_with_cache(
        ...
        # FIXED: never cache empty/blank responses (they poison the cache and turn
        # every re-run into a parse_error). Only persist a non-empty raw.
        if self.cache_dir and raw and raw.strip():
